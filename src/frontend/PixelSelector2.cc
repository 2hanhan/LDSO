...
namespace ldso
{
                                ...
                                int recursionsLeft, bool plot, float thFactor)
    {
        ...
        quotia = numWant / numHave;   //得到与想要的比例

        // 选择新的采样像素点，范围大小相当于动态网格，pot越小取点越多
        // by default we want to over-sample by 40% just to be sure.
        float K = numHave * (currentPotential + 1) * (currentPotential + 1); //相当于覆盖面积，每一个像素对应一个pot*pot
        ...
        if (recursionsLeft > 0 && quotia > 1.25 && currentPotential > 1)
        {
            ...
            if (idealPotential >= currentPotential)
                idealPotential = currentPotential - 1; //减小pot，多采集一些点
        ...
        else if (recursionsLeft > 0 && quotia < 0.25)
        {
            ...
            if (idealPotential <= currentPotential)
                idealPotential = currentPotential + 1; //增大pot，少采集一些点
            currentPotential = idealPotential;
            return makeMaps(fh, map_out, density, recursionsLeft - 1, plot, thFactor);
        }
