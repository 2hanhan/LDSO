...
namespace ldso
{
                                                ...
                                                std::string vignetteFilename)
    {
        ...
        // RadTan相机[8个]参数 内参[fx/weight] [fy/height] [cx/weight] [cy/height] 畸变 [k1] [k2] [q1] [q2]
    ...
    Undistort::undistort(const MinimalImage<T> *image_raw, float exposure, double timestamp, float factor) const
    {
        ...
        if (!passthrough)
        {
            ...
            // step 2 如果定义了噪声值，设置随机几何噪声大小，并添加到输出图像（用于实验验证对几何噪声的抵抗能力）
            float *noiseMapX = 0;
            float *noiseMapY = 0;
            ...
            for (int idx = w * h - 1; idx >= 0; idx--)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        out_data[idx] = xxyy * src[1 + wOrg] + (yy - xxyy) * src[wOrg] + (xx - xxyy) * src[1] + (1 - xx - yy + xxyy) * src[0]; //像素差值
                    }
                }
            }
    ...
    void Undistort::makeOptimalK_crop()
    {
        ...
        //矫正后映射回去看看是否在原图像上有对应的像素，
        //来更新相机minX,Y和maxX,Y范围
        for (int x = 0; x < 100000; x++)
        {
            tgX[x] = (x - 50000.0f) / 10000.0f; //-5~5，分成了100,000份
            tgY[x] = 0;
        }
    ...
    /**
     * @brief 从camera.txt中读取相机参数
     *
     * @param configFileName 相机参数txt文件
     * @param nPars 参数个数
     * @param prefix 相机模型名称
     */
    void Undistort::readFromFile(const char *configFileName, int nPars, std::string prefix)
    {
        ...
        //第一行、第二行：根据相机参数个数判断相机类型，读取分辨率和内参
        // l1 & l2
        if (nPars == 5) // fov model
        {
                ...
                std::sscanf(l2.c_str(), "%d %d", &wOrg, &hOrg) == 2)
            {
                ...
                printf("In: %f %f %f %f %f\n",
                       parsOrg[0], parsOrg[1], parsOrg[2], parsOrg[3], parsOrg[4]); //相机模型参数
            }
            else
            {
        ...
        //如果cx，cy小于1，说明是个相对值，需要乘以图像的分辨率大小
        ...
        if (outputCalibration[0] == -1)
            makeOptimalK_crop(); //求解裁切矫正后的相机内参
