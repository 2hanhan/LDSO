...
/*********************************************************************************
 * This program demonstrates how to run LDSO in KaistUrban dataset
 * Note we don't have photometric calibration in KaistUrban so we should let the algorithm
...
std::string source = "~/urban39";
std::string output_file = "./results.txt";
std::string calib = "./examples/KaistUrban/kaist.txt";
...
void parseArgument(char *arg)
{
    ...
    if (1 == sscanf(arg, "calib=%s", buf)) //从字符串中中取字符串
    {
        calib = buf; //相机标定txt文件路径
    ...
    if (1 == sscanf(arg, "files=%s", buf)) //从字符串中中取字符串
    {
        source = buf; //数据集存储路径
...
int main(int argc, char **argv)
{
    ...
    reader->setGlobalCalibration(); //设置标定参数
