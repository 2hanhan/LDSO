...
public:
    ...
    ImageFolderReader(DatasetType datasetType, //读取的数据类型
                      std::string path,        //数据集存储路径
                      std::string calibFile,   //标定txt文件存储路径
...
private:
    ...
    //加载kaist数据集
    inline void loadDatastampsKaist()
    {
        LOG(INFO) << "Loading Kaist data stamps!" << endl;
        std::ifstream file_data_stamp;
        std::string data_stamp_path = path + "/sensor_data/data_stamp.csv"; //数据<时间戳,数据类型>

        file_data_stamp.open(data_stamp_path.c_str());
        if (!file_data_stamp)
        {
            LOG(INFO) << "cannot find data stamps file at " << path + "/sensor_data/data_stamp.csv" << endl;
            return;
        }
        ...
        while (getline(file_data_stamp, line_str))
        {
            ...
            if (sensor_type == "stereo") //图像信息
            {
                img_file = path + "/image/stereo_left/" + time_str + ".png";
                timestamps.push_back(timestamp); //图像信息对应的时间戳
                files.push_back(img_file);       //图像路径
            }
            else
            {
                continue;
        ...
        file_data_stamp.close();
        LOG(INFO) << "Load total " << timestamps.size() << " data entries." << endl;
    }

    std::vector<ImageAndExposure *> preloadedImages;
    std::vector<std::string> files; //图像存储路径
    std::vector<double> timestamps; //时间戳数据
